...
public class HttpInclude {
	...
	private boolean isRemoteHttpRequest(String includePath) {
		...
		// System.out.println(!includePath.toLowerCase().startsWith(
		// "https://" + serverName));
		if (includePath.toLowerCase().startsWith("http://") && includePath.toLowerCase().startsWith("https://")
				&& !includePath.toLowerCase().startsWith("http://" + serverName)
				&& !includePath.toLowerCase().startsWith("https://" + serverName)) {
			flag = true;
		}
